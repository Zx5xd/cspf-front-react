...
import {
  ContentProps,
  QuestionsManageProps,
} from '../../components/user/props/AdminProps.ts'
import {
  axiosDelete,
  axiosGet,
  axiosPatch,
  axiosPost,
} from '../../util/axiosData.ts'
...
export const QuestionsManage: React.FC = () => {
  ...
  const commentModifyAndDelete = (id: number, type: string) => {
    ...
    if (type === 'Modify' && contentModify) {
      axiosPatch(`${cspfHost}comment/${id}`, { content: contentModify }).then(
        (data) => {
          console.log(data)
        }
      )
            ...
            <div className='ibox-content forum-container'>
              ...
              {forum.map((item) => (
                <QuesBoardItem
                  ...
                  id={item.id}
                  title={item.title}
                  content={selectQuestion?.content}
