...
import { useExpertSettings, useExpertInfo } from '@/store/useExpertStore.ts'
...
export const ExpertMyPage: React.FC = () => {
  ...
  const handleDetailProfileInputChange = (
  ...
  const handleSave = async () => {
    try {
      ...
      await axiosPatch(
        `${migHost()}expert/${users?.expertCode}`,
        ...
        .then((res) => {
          if (res && res.status === 200) {
            users === res.data
              ? alert('업데이트 에러...')
              : (setUsers(res.data), setShowModal(false))
          }
        })
  ...
  return (
    <>
      ...
      <DetailProfileSetModal
        ...
        handleSave={handleDetailProfileSave}
        formData={detailProfile}
