...
const MessageList: React.FC<MessageListProps> = ({ messages, roomId }) => {
  const expertInfo = useExpertInfo()?.expertCode
  const scrollContainerRef = useRef<HTMLDivElement | null>(null)
  const msgRef = useRef<HTMLDivElement[] | null>([])
  const msgStartRef = useRef<HTMLDivElement | null>(null)
  const isType = expertInfo?.charAt(0) === 'U' ? 'USER' : 'EXPERT'

  const { chatLog, isFetching, logLength } = useChatLogList(
    roomId,
    scrollContainerRef
  )
  ...
  useEffect(() => {
    console.log('userCode, roomId', expertInfo, roomId)
  }, [])
  ...
  useEffect(() => {
    ...
    msgRef.current[logLength - 19]?.scrollIntoView({
      behavior: 'auto',
      block: 'start',
    })
