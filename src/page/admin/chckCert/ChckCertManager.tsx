...
import {chckCertColumns} from "@/page/admin/chckCert/Columns.tsx";
...
function getData(): ExpertEntity[] {
  ...
  const [chckList, setChckList] = useState<ExpertEntity[]>([])

  useEffect(() => {
    expertInfo().then(data => {
      const ftData = data.filter((expert)=>
      expert.credentialStatus === 0
      )
      console.log(ftData);
      setChckList(ftData)
      })}, []);
    ...
    <div className={"sm:container sm:mx-auto h-screen py-8 bg-white/5"}>
      <main className="flex flex-row h-full overflow-hidden">
        <div className="flex flex-col h-full backdrop-brightness-50 mr-2 rounded-lg w-64">
          ...
          <Button variant="secondary" className="mx-2 my-2" onClick={() => navigate('/admin/chckCert')}>자격증</Button>
        </div>
        <DataTable columns={chckCertColumns} data={data}/>
      </main>
    </div>
  )
}